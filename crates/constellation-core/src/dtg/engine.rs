...
use crate::dtg::error::{DtgError, DtgResult, validation_errors};
...
pub struct DtgExecutionEngine {
    ...
    node_executor: Box<dyn Fn(&mut DtgNode) -> DtgResult<DtgMetrics> + Send + Sync>,
}
...
impl DtgExecutionEngine {
    ...
    pub fn new(
        ...
        if !graph.is_acyclic() {
            return Err(DtgError::CyclicGraph(
                "Cannot execute DTG with cycles. Use validate() to check for cycles first."
                    .to_string(),
            ));
        }
    ...
    pub fn execute(&mut self) -> DtgResult<()> {
    ...
    pub fn execute_step(&mut self) -> DtgResult<()> {
    ...
    pub fn validate(&self) -> DtgResult<()> {
        let mut errors = Vec::new();

        // Validate graph structure and data
        if let Err(validation_errors) = self.graph.validate_graph() {
            errors.push(format!("Graph validation failed: {validation_errors}"));
        }
        ...
        for node_id in self.graph.nodes.keys() {
            ...
            if !has_incoming && !has_outgoing && !self.graph.root_nodes.contains(node_id) {
                errors.push(format!("Node {node_id} is orphaned (no connections)"));
            }
        }
    ...
    /// Find cycles in the DTG using petgraph's strongly connected components.
    pub fn find_cycles(&self) -> Vec<Vec<Uuid>> {
        ...
        // Find strongly connected components (SCCs) with more than one node
        let sccs = petgraph::algo::kosaraju_scc(&graph);

        for scc in sccs {
            if scc.len() > 1 {
                // Convert NodeIndex back to Uuid
                let cycle: Vec<Uuid> = scc.iter().map(|&idx| graph[idx]).collect();
                cycles.push(cycle);
            }
        }
    ...
    fn update_completed_nodes(&mut self) {
        let completed_nodes: Vec<Uuid> = self
            .executing
            .iter()
            .filter(|node_id| {
                ...
                matches!(
                    node.status,
                    DtgNodeStatus::Completed | DtgNodeStatus::Failed
                )
            })
            .cloned()
            .collect();
    ...
    fn execute_ready_nodes(&mut self) -> DtgResult<()> {
        while let Some(node_id) = self.ready_queue.pop_front() {
            ...
            let all_deps_completed = dependencies
                .iter()
            ...
            if let Some(node) = self.graph.nodes.get_mut(&node_id) {
                ...
                match (self.node_executor)(node) {
                    ...
                    Err(error) => {
                        node.mark_failed(error.to_string());
                        self.failed.insert(node_id);
                        self.executing.remove(&node_id);
                        return Err(DtgError::NodeExecutionFailed(node_id, error.to_string()));
                    }
                }
            } else {
                return Err(DtgError::NodeNotFound(node_id));
            }
        }
...
mod tests {
    ...
    fn test_engine_creation() {
        ...
        let engine = DtgExecutionEngine::new(graph, executor).unwrap();
