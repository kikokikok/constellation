...
impl DtgMetricsCollector {
    ...
    fn compute_performance_score(&self, metrics: &DtgMetrics) -> f64 {
        ...
        let memory_score =
            self.normalize_metric(metrics.memory_bytes as f64, 0.0, 1_000_000_000.0, true); // 1GB
        let latency_score =
            self.normalize_metric(metrics.execution_time_ms as f64, 0.0, 10000.0, true);

        // Throughput (higher is better)
        let throughput_score =
            self.normalize_metric(metrics.throughput_ops_per_sec, 0.0, 1000.0, false);
    ...
    fn compute_confidence_score(&self, metrics: &DtgMetrics, skill_id: &str) -> f64 {
        if let Some(historical) = self.historical_metrics.get(skill_id) {
            ...
            let avg_historical_quality: f64 =
                historical.iter().map(|m| m.quality_score).sum::<f64>() / historical.len() as f64;
    ...
    fn normalize_metric(&self, value: f64, min: f64, max: f64, invert: bool) -> f64 {
        ...
        if invert { 1.0 - clamped } else { clamped }
    }
    ...
    fn update_baseline(&mut self, metrics: &DtgMetrics) {
        if self.baseline_metrics.is_none() {
            ...
            baseline.cpu_time_ms = ((1.0 - alpha) * baseline.cpu_time_ms as f64
                + alpha * metrics.cpu_time_ms as f64) as u64;
            baseline.memory_bytes = ((1.0 - alpha) * baseline.memory_bytes as f64
                + alpha * metrics.memory_bytes as f64) as u64;
            baseline.execution_time_ms = ((1.0 - alpha) * baseline.execution_time_ms as f64
                + alpha * metrics.execution_time_ms as f64)
                as u64;
            baseline.throughput_ops_per_sec = (1.0 - alpha) * baseline.throughput_ops_per_sec
                + alpha * metrics.throughput_ops_per_sec;
            baseline.quality_score =
                (1.0 - alpha) * baseline.quality_score + alpha * metrics.quality_score;
        }
    }
    ...
    pub fn analyze_node_performance(&self, node: &DtgNode) -> PerformanceAnalysis {
        ...
        if metrics.memory_bytes > 100_000_000 {
            ...
            recommendations
                .push("Consider implementing memory pooling or streaming processing".to_string());
        }
        ...
        if metrics.error_rate > 0.1 {
            ...
            recommendations.push(
                "Improve error handling and add retry logic with exponential backoff".to_string(),
            );
        }
        ...
        if metrics.retry_count > 3 {
            ...
            recommendations
                .push("Investigate root cause of failures and improve stability".to_string());
        }
        ...
        if metrics.business_value_score < 0.7 {
            ...
            recommendations.push(
                "Re-evaluate transformation goals and alignment with business objectives"
                    .to_string(),
            );
        }
    ...
    pub fn generate_quality_report(&self, nodes: &[&DtgNode]) -> QualityReport {
        ...
        let avg_quality: f64 =
            analyses.iter().map(|a| a.quality_score).sum::<f64>() / analyses.len() as f64;
