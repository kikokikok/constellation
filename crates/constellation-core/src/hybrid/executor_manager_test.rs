...
use crate::models::hybrid_agent::{
    ExecutorConfig, ExecutorDomain, ExecutorModel, ExecutorPerformance, ExecutorSkill,
    ExecutorModelSize, ModelProvider, ResourceRequirements,
};
...
fn test_executor_state_management() {
    ...
    assert!(manager.update_executor_state("test-executor-2", ExecutorLifecycleState::Ready).is_ok());
    assert!(manager.update_executor_state("test-executor-2", ExecutorLifecycleState::Busy).is_ok());
    assert!(manager.update_executor_state("test-executor-2", ExecutorLifecycleState::Draining).is_ok());
...
fn test_executor_health_monitoring() {
    ...
    manager.register_executor(config).unwrap();
    manager.update_executor_state("test-executor-3", ExecutorLifecycleState::Ready).unwrap();
    ...
    assert!(manager.update_executor_health("test-executor-3", health_update).is_ok());
    ...
    for _ in 0..3 {
        ...
        manager.update_executor_health("test-executor-3", health_update).unwrap();
    }
...
fn test_executor_load_management() {
    ...
    manager.register_executor(config).unwrap();
    manager.update_executor_state("test-executor-4", ExecutorLifecycleState::Ready).unwrap();
    ...
    // Complete tasks
    assert!(manager.update_executor_load("test-executor-4", true, 1000).is_ok());
    let executor = manager.get_executor("test-executor-4").unwrap();
...
fn test_executor_matching() {
    ...
    manager.update_executor_state("code-executor", ExecutorLifecycleState::Ready).unwrap();
    manager.update_executor_state("data-executor", ExecutorLifecycleState::Ready).unwrap();
...
fn test_scaling_recommendations() {
    ...
    manager.register_executor(config).unwrap();
    manager.update_executor_state("test-executor-5", ExecutorLifecycleState::Ready).unwrap();
...
fn test_performance_snapshots() {
    ...
    manager.register_executor(config).unwrap();
    manager.update_executor_state("test-executor-6", ExecutorLifecycleState::Ready).unwrap();
