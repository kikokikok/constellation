...
use crate::models::hybrid_agent::{
    ExecutorConfig, ExecutorDomain, ExecutorPerformance,
};
...
impl SlmExecutorManager {
    ...
    pub fn register_executor(&self, config: ExecutorConfig) -> Result<String, String> {
        ...
        if executors.contains_key(&executor_id) {
            return Err(format!("Executor with ID '{}' already registered", executor_id));
        }
    ...
    pub fn update_executor_state(&self, executor_id: &str, state: ExecutorLifecycleState) -> Result<(), String> {
        let mut executors = self.executors.lock().unwrap();
        ...
        match executors.get_mut(executor_id) {
            ...
            None => Err(format!("Executor with ID '{}' not found", executor_id)),
        }
    }
    
    /// Update executor health status.
    pub fn update_executor_health(&self, executor_id: &str, health_update: ExecutorHealthUpdate) -> Result<(), String> {
        let mut executors = self.executors.lock().unwrap();
    ...
    pub fn update_executor_load(&self, executor_id: &str, task_completed: bool, execution_time_ms: u64) -> Result<(), String> {
        let mut executors = self.executors.lock().unwrap();
        ...
        match executors.get_mut(executor_id) {
            Some(executor) => {
                ...
                if executor.active_task_count >= executor.config.max_concurrent_tasks {
                    return Err(format!("Executor '{}' at maximum capacity", executor_id));
                }
                ...
                match executor.lifecycle_state {
                    ...
                    _ => Err(format!("Executor '{}' not in ready state: {:?}", executor_id, executor.lifecycle_state)),
                }
            }
            None => Err(format!("Executor with ID '{}' not found", executor_id)),
        }
        ...
        for executor in executors.values() {
            ...
            if match_score > 0.0 {
                let executor_match = ExecutorMatch {
                    ...
                    match_score,
                    domain_match_score: self.calculate_domain_match_score(&executor.specialization, criteria),
                    skill_match_score: self.calculate_skill_match_score(&executor.specialization, criteria),
                    performance_match_score: self.calculate_performance_match_score(&executor.config.performance, criteria),
                    cost_match_score: self.calculate_cost_match_score(&executor.config.performance, criteria),
    ...
    fn calculate_match_score(&self, executor: &ManagedExecutor, criteria: &MatchingCriteria) -> f64 {
        ...
        let performance_score = self.calculate_performance_match_score(&executor.config.performance, criteria);
        ...
        let weights = match &self.load_balancing_strategy {
            ...
            LoadBalancingStrategy::Hybrid { load_weight, performance_weight, cost_weight, latency_weight } => {
    ...
    fn calculate_domain_match_score(&self, specialization: &ExecutorSpecialization, criteria: &MatchingCriteria) -> f64 {
    ...
    fn calculate_skill_match_score(&self, specialization: &ExecutorSpecialization, criteria: &MatchingCriteria) -> f64 {
        ...
        for required_skill in &criteria.required_skills {
            if let Some(proficiency) = specialization.skills.get(required_skill) {
                if *proficiency >= criteria.min_skill_proficiency {
    ...
    fn calculate_performance_match_score(&self, performance: &ExecutorPerformance, criteria: &MatchingCriteria) -> f64 {
        let mut score = 1.0;
    ...
    fn calculate_cost_match_score(&self, performance: &ExecutorPerformance, criteria: &MatchingCriteria) -> f64 {
    ...
    fn calculate_health_score(&self, health: &ExecutorHealth) -> f64 {
        ...
        let stale_secs = now.signed_duration_since(health.last_heartbeat).num_seconds();
    ...
    pub fn check_scaling_recommendations(&self) -> ScalingRecommendation {
        ...
        {
            let count_to_add = ((avg_load - self.scaling_thresholds.scale_up_load_threshold) * total_executors as f64).ceil() as u32;
            ...
            return ScalingRecommendation::ScaleUp {
                ...
                domain,
                reason: format!("High average load: {:.2}", avg_load),
            };
        }
        ...
        {
            let count_to_remove = ((self.scaling_thresholds.scale_down_load_threshold - avg_load) * total_executors as f64).ceil() as u32;
            ...
            return ScalingRecommendation::ScaleDown {
                ...
                executor_ids,
                reason: format!("Low average load: {:.2}", avg_load),
            };
        }
            ...
            && total_executors < self.scaling_thresholds.max_executors
        {
            ...
            return ScalingRecommendation::ScaleUp {
                count: 1,
                domain,
                reason: format!("Low success rate: {:.2}", avg_success_rate),
            };
        }
            ...
            && total_executors < self.scaling_thresholds.max_executors
        {
            ...
            return ScalingRecommendation::ScaleUp {
                count: 1,
                domain,
                reason: format!("High latency: {:.0}ms", avg_latency),
            };
        }
    ...
    fn get_most_loaded_domain(&self) -> ExecutorDomain {
        ...
        for executor in executors.values() {
            if self.can_accept_tasks(executor) {
                let entry = domain_loads.entry(executor.specialization.primary_domain.clone()).or_insert((0.0, 0));
    ...
    pub fn record_performance_snapshot(&self) {
        ...
        for executor in executors.values() {
            if self.can_accept_tasks(executor) {
                ...
                total_available_capacity += executor.config.max_concurrent_tasks - executor.active_task_count;
                count += 1;
