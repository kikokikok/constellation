...
use crate::models::hybrid_agent::{FallbackAction, FallbackStrategy, FallbackTrigger, HybridAgentConfig};
use serde_json::json;

#[test]
fn test_coordinator_creation() {
    let config = HybridAgentConfig::new(
...
fn test_fallback_conditions() {
    let mut config = HybridAgentConfig::new(
        "Test Agent".to_string(),
        "Test description".to_string(),
    );
    ...
    let coordinator = LlmStrategistCoordinator::new(config)
        .with_fallback_strategies(fallback_strategies);
