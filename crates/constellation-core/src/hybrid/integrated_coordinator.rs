...
use crate::hybrid::resource_manager::{
    AllocationResult, ResourceManager, ResourceRequest, QualityRequirements,
    ScalingOperation, ScalingResult,
};
use crate::models::hybrid_agent::{ExecutorConfig, ExecutorDomain, FallbackStrategy, HybridAgentConfig};
...
impl IntegratedCoordinator {
    ...
    pub fn register_executor(&self, config: ExecutorConfig) -> Result<String, String> {
        ...
        self.executor_manager.update_executor_state(&executor_id, ExecutorLifecycleState::Ready)?;
        
        Ok(executor_id)
    }
    
    /// Update executor status (compatible with base coordinator interface).
    pub fn update_executor_status(&self, status: crate::hybrid::coordinator::ExecutorStatus) -> Result<(), String> {
        ...
        let _ = self.executor_manager.update_executor_health(&executor_id, health_update);
    ...
    fn task_to_resource_request(&self, task: &Task) -> Result<ResourceRequest, String> {
        ...
        let domain = self.infer_domain_from_task_type(&task.task_type)
            .map(|d| format!("{:?}", d))
    ...
    pub fn complete_task(&self, result: TaskResult) -> Result<(), String> {
        ...
        let _ = self.executor_manager.update_executor_health(&result.executor_id, health_update);
        
        // Update executor load
        self.executor_manager.update_executor_load(&result.executor_id, task_completed, execution_time_ms)?;
    ...
    pub fn get_combined_performance_metrics(&self) -> (PerformanceMetrics, Vec<crate::hybrid::executor_manager::PerformanceSnapshot>) {
    ...
    pub fn get_executor_stats(&self) -> (Vec<ExecutorStats>, Vec<crate::hybrid::executor_manager::ManagedExecutor>) {
    ...
    pub fn check_and_apply_auto_scaling(&self) -> Vec<ScalingResult> {
        ...
        if !scaling_operations.is_empty() {
            self.resource_manager.apply_scaling_operations(scaling_operations)
        } else {
            Vec::new()
        }
    }
    ...
    pub fn get_resource_utilization_stats(&self) -> crate::hybrid::resource_manager::UtilizationStats {
        self.resource_manager.get_utilization_stats()
    }
    
    /// Optimize resource allocation.
    pub fn optimize_resource_allocation(&self) -> crate::hybrid::resource_manager::OptimizationRecommendations {
        self.resource_manager.optimize_allocation()
    }
    
    /// Update resource allocation configuration.
    pub fn update_resource_allocation_config(&mut self, config: crate::models::hybrid_agent::ResourceAllocation) {
    ...
    pub fn release_resources_for_task(&self, task_id: Uuid, executor_id: &str) -> Result<(), String> {
        // Simplified implementation
        let requirements = crate::models::hybrid_agent::ResourceRequirements::default();
        self.resource_manager.release_resources(executor_id, &requirements);
    ...
    pub fn get_performance_history(&self) -> crate::hybrid::performance_monitor::PerformanceHistory {
    ...
    pub fn get_performance_trends(&self, window: Duration) -> crate::hybrid::performance_monitor::PerformanceTrends {
        self.performance_monitor.get_performance_trends(window)
    }
    
    /// Get optimization recommendations.
    pub fn get_optimization_recommendations(&self) -> Vec<crate::hybrid::performance_monitor::OptimizationResult> {
        self.performance_monitor.get_optimization_recommendations()
    }
    
    /// Get scaling recommendations.
    pub fn get_scaling_recommendations(&self) -> Option<crate::hybrid::performance_monitor::ScalingRecommendation> {
        self.performance_monitor.get_scaling_recommendations()
    }
    
    /// Add an alert subscriber.
    pub fn add_alert_subscriber(&self, subscriber: crate::hybrid::performance_monitor::AlertSubscriber) {
    ...
    pub async fn handle_alert_with_fallback(&self, alert: &crate::hybrid::performance_monitor::Alert) -> Vec<crate::models::hybrid_agent::FallbackAction> {
    ...
    where
        ...
        T: Send + 'static,
    {
        self.fallback_manager.execute_with_timeout(operation, timeout).await
    }
    
    /// Add a circuit breaker for an executor or service.
    pub async fn add_circuit_breaker(&self, id: String, config: Option<crate::hybrid::fallback_manager::CircuitBreakerConfig>) {
    ...
    pub async fn record_circuit_breaker_success(&self, circuit_breaker_id: &str) {
        self.fallback_manager.record_success(circuit_breaker_id).await;
    }
    
    /// Record failure for a circuit breaker.
    pub async fn record_circuit_breaker_failure(&self, circuit_breaker_id: &str) {
        self.fallback_manager.record_failure(circuit_breaker_id).await;
    }
    
    /// Check if a circuit breaker is available.
    pub async fn is_circuit_breaker_available(&self, circuit_breaker_id: &str) -> bool {
        self.fallback_manager.is_circuit_breaker_available(circuit_breaker_id).await
    }
    
    /// Add a bulkhead for failure isolation.
    pub async fn add_bulkhead(&self, id: String, config: Option<crate::hybrid::fallback_manager::BulkheadConfig>) {
    ...
    pub async fn get_current_degradation_level(&self) -> crate::hybrid::fallback_manager::DegradationLevel {
        self.fallback_manager.get_current_degradation_level().await
    }
    
    /// Get recommended fallback actions for a trigger.
    pub async fn get_recommended_fallback_actions(&self, trigger: crate::models::hybrid_agent::FallbackTrigger) -> Vec<crate::models::hybrid_agent::FallbackAction> {
    ...
    pub async fn check_and_handle_fallback_conditions(&self) -> Vec<crate::models::hybrid_agent::FallbackAction> {
