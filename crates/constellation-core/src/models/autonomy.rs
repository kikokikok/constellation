...
impl CapabilityAxis {
    ...
    pub fn description(&self) -> &'static str {
        match self {
            ...
            CapabilityAxis::ResourceManagement => "Ability to manage computational and physical resources",
            CapabilityAxis::MetaCognition => "Ability to reflect on and improve own cognitive processes",
        }
    }
}
...
impl CollaborationPatternType {
    ...
    pub fn description(&self) -> &'static str {
        match self {
            CollaborationPatternType::Hierarchical => "Clear leader-follower structure with centralized decision-making",
            CollaborationPatternType::Distributed => "Equal peers with distributed decision-making and coordination",
            CollaborationPatternType::Specialized => "Agents with complementary specialized skills working together",
            CollaborationPatternType::Adaptive => "Structure changes dynamically based on task requirements",
            CollaborationPatternType::Swarm => "Many simple agents achieving complex emergent behavior",
            CollaborationPatternType::MarketBased => "Resource allocation and task assignment through bidding/auction mechanisms",
            CollaborationPatternType::Consensus => "Decisions made through consensus mechanisms among agents",
            CollaborationPatternType::MentorApprentice => "Knowledge transfer between experienced and learning agents",
            CollaborationPatternType::Competitive => "Agents compete to improve overall system performance",
...
impl CollaborationPattern {
    ...
    pub fn update_with_observation(&mut self, success: bool, efficiency: f64, duration: Duration) {
        ...
        self.efficiency = (self.efficiency * (total_observations - 1) as f64 + efficiency) / total_observations as f64;
...
impl AutonomyBenchmark {
    ...
    pub fn validate_measurement(&self, measurement: &AutonomyMeasurement) -> BenchmarkValidationResult {
        let mut axis_validations = HashMap::new();
        
        // Validate each capability axis
        for axis in CapabilityAxis::all() {
            if let Some(expected_scores) = self.expected_kappa_scores.get(&measurement.autonomy_level) {
                if let Some(expected_score) = expected_scores.get(&axis) {
                    if let Some(actual_score) = measurement.get_kappa_score(axis) {
                        let discrepancy = (actual_score.score - expected_score).abs();
                        let is_within_tolerance = discrepancy <= 0.1; // 10% tolerance
                        
                        axis_validations.insert(axis, AxisValidation {
        ...
        let validation_score = if axis_validations.is_empty() {
            ...
            let valid_count = axis_validations.values().filter(|v| v.is_within_tolerance).count();
        ...
        BenchmarkValidationResult {
            ...
            environment_complexity_match: self.environment_complexities.contains(&measurement.environment_complexity),
...
impl AutonomyProgress {
    ...
    pub fn new(agent_id: String, starting_level: AutonomyLevel, target_level: AutonomyLevel) -> Self {
        let now = SystemTime::now();
        ...
        if let Ok(duration) = last.timestamp.duration_since(first.timestamp) {
            ...
            if days > 0.0 {
                ...
                if self.progress_rate > 0.0 {
                    ...
                    self.estimated_time_to_target = Some(Duration::from_secs((days_to_target * 86400.0) as u64));
                }
