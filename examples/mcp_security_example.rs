...
fn main() {
    ...
    println!(
        "  Signature Algorithm: {}",
        medium_context.algorithms.signature
    );
    println!(
        "  Encryption Algorithm: {}",
        medium_context.algorithms.encryption
    );
    println!("  Key Storage: {:?}", medium_context.key_management.storage);
    println!(
        "  Authentication: {:?}",
        medium_context.access_control.authentication
    );
    ...
    println!(
        "  Signature Algorithm: {}",
        high_context.algorithms.signature
    );
    println!(
        "  Encryption Algorithm: {}",
        high_context.algorithms.encryption
    );
    println!("  Key Storage: {:?}", high_context.key_management.storage);
    println!(
        "  Authentication: {:?}",
        high_context.access_control.authentication
    );
    ...
    println!(
        "  GDPR compliant: {}",
        compliance_context.is_compliant(ComplianceStandard::Gdpr, "GDPR-25")
    );
    println!(
        "  ISO27001 compliant: {}",
        compliance_context.is_compliant(ComplianceStandard::Iso27001, "A.9.2.1")
    );
    ...
    for event in &high_context.audit_logging.events_to_log {
        ...
        println!(
            ...
            event.log_success, event.log_failure
        );
    }
